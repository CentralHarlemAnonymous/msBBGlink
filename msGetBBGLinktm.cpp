...
# line 2 "msGetBBGLink.tm"
void msGetBBG P(( int, int));

# line 187 "msGetBBGLinktm.cpp"


void msGetBBGLink P(( const char * _tp1, const char * _tp2, int _tp3, const char * _tp4));
...
#endif
{
	...
	msGetBBGLink(_tp1, _tp2, _tp3, _tp4);

	res = 1;
L4:	WSReleaseString(mlp, _tp4);
L3: L2:	WSReleaseString(mlp, _tp2);
...
static struct func {
	...
	const char  *f_name;
		...
		{ 4, 0, _tr0, "msGetBBGLink" }
		};

static const char* evalstrs[] = {
	"msGetBBGLink::usage = \"msGetBBGLink[ticker, field, noisy, overri",
	"des] returns the requested fields for the specified tickers by d",
	"irect call to WSTP. If noisy is set to True, outputs debugging i",
	"nformation.\"",
	(const char*)0,
	(const char*)0
};
...
#endif
{
	int _res;
	_res = WSConnect(mlp);
	if (_res) _res = _definepattern(mlp, (char *)"msGetBBGLink[ticker_String, field_String, noisy_Integer, overrides_String]", (char *)"{ ticker, field, noisy, overrides }", 0);
