 ...
 *   In[1]:= link = Install["msBBGcurrentLink.exe"]
 *
...
public:
	...
	// makes one or more Bloomberg calls, gets the Event response and associated MessageIterator, then calls ProcessMessage() to deal with what it's gotten. Returns the result to msBBGcurrentLink()
	// closely related to the "Retrieve" function in the 2008/2009 code's BBRetrieveCurrent
	std::string dataRequest(const char* Ticker, const char* Field, int debug, const char* overrides)
	{
		std::stringstream outstream;
...
// per the template file, msBBGcurrentLink() has a returntype of Manual, which means I need to use a function like WSPutInteger32() to return an integer
void msBBGcurrentLink(const char* ticker, const char* field, int debug, const char* overrides)
{
	std::string bbgOutput;
	BBGdataObject bbgDataObj;
	bbgOutput = bbgDataObj.dataRequest(ticker, field, debug, overrides);
